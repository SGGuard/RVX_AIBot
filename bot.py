...
async def send_expert_response(
    ...
    """
    Отправляет развёрнутый экспертный ответ с интерактивностью.
    
    Это главная функция для **информативных, развёрнутых ответов**.
    
    Args:
        update: Telegram Update
        title: Заголовок (основная тема)
        main_content: Основное содержание (подробное объяснение)
        context: Контекст и фон
        key_points: Ключевые моменты (список)
        examples: Примеры (список с реальными case'ами)
        interactive_questions: Вопросы для размышления пользователя
        related_topics: Связанные темы для дальнейшего изучения
        next_steps: Следующие шаги в обучении
    ...
    message += f"<b>📖 Подробно:</b>\n{main_content}\n\n"
    
    # Контекст и фон
    if context:
        message += f"<b>🔍 Контекст:</b>\n{context}\n\n"
    ...
    # Примеры
    if examples:
        message += "<b>💼 Реальные примеры:</b>\n"
        for i, example in enumerate(examples, 1):
            message += f"  Пример {i}: {example}\n"
        message += "\n"
    
    # Вопросы для размышления (ОЧЕНЬ ВАЖНО!)
    if interactive_questions:
        message += "<b>🤔 Вопросы для размышления:</b>\n"
    ...
    # Связанные темы
    if related_topics:
        message += "<b>🔗 Связанные темы для изучения:</b>\n"
    ...
    message += f"\n<i>💭 Эта информация поможет тебе глубже разобраться в теме.</i>"
...
async def send_analytical_breakdown(
    ...
    """
    Отправляет аналитический разбор с глубоким анализом.
    
    Для новостей, анализов, сложных тем.
    
    Args:
        update: Telegram Update
        topic: Название темы
        analysis: Основной анализ
        pros: Положительные аспекты
        cons: Отрицательные аспекты
        critical_points: Критические моменты
        comparison: Сравнение с альтернативами
        statistical_data: Статистические данные
        expert_opinion: Мнение экспертов
        discussion_prompts: Вопросы для дискуссии
        
    Returns:
        None
    """
    message = f"<b>📊 АНАЛИТИЧЕСКИЙ РАЗБОР: {topic}</b>\n"
    message += "=" * 50 + "\n\n"
    ...
    # Плюсы
    if pros:
        message += "<b>✅ Положительные аспекты:</b>\n"
    ...
    # Минусы
    if cons:
        message += "<b>⚠️ Отрицательные аспекты:</b>\n"
    ...
    # Критические моменты
    if critical_points:
        message += "<b>🚨 КРИТИЧЕСКИЕ МОМЕНТЫ (важно!):</b>\n"
    ...
    # Сравнение
    if comparison:
        message += f"<b>⚖️ Сравнение:</b>\n{comparison}\n\n"
    
    # Статистика
    if statistical_data:
        message += f"<b>📈 Статистические данные:</b>\n{statistical_data}\n\n"
    
    # Мнение экспертов
    if expert_opinion:
        message += f"<b>👨‍💼 Мнение экспертов:</b>\n{expert_opinion}\n\n"
    
    # Вопросы для дискуссии
    if discussion_prompts:
        message += "<b>💬 Давай обсудим (напиши ответ):</b>\n"
        for i, prompt in enumerate(discussion_prompts, 1):
            message += f"  {i}. {prompt}\n"
        message += "\n"
    
    message += "<i>🎓 Будь критичным в оценке информации!</i>"
...
async def send_interactive_learning(
    ...
    """
    Отправляет интерактивный урок с заданиями и вопросами.
    
    Для полноценного обучения, а не просто ответов.
    ...
    Args:
        ...
        common_mistakes: Типичные ошибки новичков
        deeper_dive: Глубокое погружение для продвинутых
        student_reflection: Вопросы для саморефлексии
        
    Returns:
        None
    """
    message = f"<b>📚 ИНТЕРАКТИВНЫЙ УРОК: {lesson_title}</b>\n"
    message += "=" * 50 + "\n\n"
    ...
    # Основные концепции
    message += "<b>🎯 Основные концепции:</b>\n"
    ...
    # Глубокое погружение
    if deeper_dive:
        message += f"<b>🔬 Глубокое погружение (для продвинутых):</b>\n{deeper_dive}\n\n"
    
    # Саморефлексия
    if student_reflection:
        message += "<b>🤔 Вопросы для саморефлексии:</b>\n"
        for q in student_reflection:
            message += f"  • {q}\n"
        message += "\n"
    
    message += "<i>💪 Учись через активное мышление, не пассивное чтение!</i>"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    """
    help_text = (
        "<b>📖 ПОЛНОЕ РУКОВОДСТВО ПО RVX BOT v0.26.3</b>\n"
        "=" * 50 + "\n\n"
        
        "<b>🎯 ЧТО МОЖЕТ ДЕЛАТЬ БОТ:</b>\n"
        "1. 📰 Анализировать криптоновости и тренды\n"
        "2. 🎓 Обучать (интерактивные уроки и курсы)\n"
        "3. 🎮 Геймифицировать обучение (квесты, достижения)\n"
        "4. 📊 Отслеживать твой прогресс\n"
        "5. 💬 Вести интерактивные диалоги\n\n"
        
        "<b>🚀 ГЛАВНЫЕ КОМАНДЫ:</b>\n\n"
        
        "<b>/teach [тема] [уровень]</b>\n"
        "   → Интерактивный учитель по крипто, AI, Web3, трейдингу\n"
        "   Темы: crypto_basics | trading | web3 | ai | defi | nft | security | tokenomics\n"
        "   Уровни: beginner 🌱 | intermediate 📚 | advanced 🚀 | expert 💎\n"
        "   Пример: /teach crypto_basics beginner\n\n"
        
        "<b>/learn</b>\n"
        "   → Структурированные образовательные курсы\n"
        "   • Blockchain основы\n"
        "   • DeFi контракты\n"
        "   • Масштабирование и DAO\n\n"
        
        "<b>/tasks</b>\n"
        "   → Ежедневные квесты для обучения\n"
        "   • Получай XP за правильные ответы\n"
        ...
        "   • Пройденные уроки\n"
        "   • Заработанные бейджи\n"
        "   • Рейтинг среди других\n\n"
        
        "<b>📰 АНАЛИЗ НОВОСТЕЙ:</b>\n"
        "   Просто отправь текст криптоновости → получишь:\n"
        ...
        "   /lesson — продолжить текущий урок\n"
        "   /history — последние анализы\n"
        "   /search [текст] — поиск в истории\n"
        "   /bookmarks — сохраненные анализы\n"
        ...
        "<b>⚡ ТВОИ ЛИМИТЫ:</b>\n"
        f"   • {MAX_REQUESTS_PER_DAY} запросов в день (считаются анализы)\n"
        f"   • {FLOOD_COOLDOWN_SECONDS}с между запросами\n"
        f"   • Макс. текст: {MAX_INPUT_LENGTH} символов\n"
        "   • Курсы и уроки НЕ считаются в лимиты!\n\n"
        
        "<b>💡 ПОЛЕЗНЫЕ СОВЕТЫ:</b>\n"
        "   ✓ Начни с /teach для интерактивного обучения\n"
        "   ✓ Пройди /tasks квесты для получения XP\n"
        "   ✓ Используй /learn для углубленного изучения\n"
        "   ✓ Сохраняй интересные анализы в /bookmarks\n"
        "   ✓ Повышай уровень - получишь больше лимитов!\n\n"
        
        "<b>🎯 ПРИМЕРЫ ИСПОЛЬЗОВАНИЯ:</b>\n"
        "   1. /teach crypto_basics — основы крипто\n"
        "   2. /teach trading beginner — трейдинг для начинающих\n"
        "   3. /teach web3 advanced — Web3 продвинутый курс\n"
        "   4. /tasks — пройти ежедневный квест\n"
        "   5. /stats — проверить свой прогресс\n\n"
        
        "<b>❓ ЧАСТО ЗАДАВАЕМЫЕ ВОПРОСЫ:</b>\n"
        "   Q: Как повысить лимиты?\n"
        "   A: Повышай уровень через XP в курсах и квестах!\n\n"
        "   Q: Зачем сохранять в закладки?\n"
        "   A: Для повторения и глубокого изучения позже!\n\n"
        "   Q: Могу ли я учиться без лимитов?\n"
        "   A: Да! Все курсы, уроки и квесты БЕЗ лимитов!\n\n"
        
        "📧 <b>Нужна помощь?</b> Напиши администратору!"
    )
