...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("clarify_"):
        ...
        last_answer = context.user_data.get("last_answer", "")  # Сохраняем последний ответ чтобы не повторять
        ...
        try:
            # Генерируем разные "углы зрения" для более глубокого понимания темы
            # Каждый клик = новый интересный аспект БЕЗ требования писать новый вопрос
            deep_dives = [
                ("🔍 Суть в 2 словах", "Объясни суть {} в 2-3 словах максимально просто. Основная идея."),
                ("💡 Практический пример", "Дай конкретный, реальный пример как {} работает в жизни людей."),
                ("📈 Как это влияет", "Как {} влияет на обычного человека или экономику? Какие последствия?"),
                ("🤔 Частые ошибки", "Какие самые частые неправильные представления о {}? Чего люди не понимают?"),
                ("🎯 Когда это полезно", "В каких ситуациях {} действительно полезен или важен?"),
                ("⚡ Ключевой момент", "Какой один ключевой момент люди должны знать о {}?"),
                ("🌍 Глобальный масштаб", "Как {} работает в мировом масштабе? Примеры из разных стран."),
                ("💰 Финансовый аспект", "Какой финансовый аспект {} важен знать? Кто зарабатывает/теряет?")
            ]
            
            # Выбираем следующий "угол зрения" на основе кол-ва кликов
            dive_index = clarify_count % len(deep_dives)
            dive_emoji, dive_template = deep_dives[dive_index]
            clarify_count += 1
            
            # Получаем ответ от ИИ - автоматически выдаем интересную информацию
            from ai_dialogue import get_ai_response_sync
            
            # Формируем запрос который автоматически генерирует интересную информацию
            # БЕЗ требования пользователю писать что-то новое
            exploration_question = (
                f"ТЕМА: {last_question}\n\n"
                f"Сейчас пользователь хочет глубже разобраться в этой теме.\n"
                f"Помоги ему понять {dive_template.format(last_question)}\n\n"
                f"⚠️ КРИТИЧНО:\n"
                f"1. Отвечай на вопрос АВТОМАТИЧЕСКИ - не просй пользователя писать\n"
                f"2. Это не обычный Q&A, это дополнительная информация для погружения\n"
                f"3. НЕ повторяй то что уже было сказано в основном ответе\n"
                f"4. Объясняй простыми словами, используй примеры\n"
                f"5. Максимум 450 символов - компактно и интересно\n"
                f"6. Используй USD/EUR в примерах (не рубли)"
            )
            
            dialogue_context = None  # Без контекста, чтобы AI сосредоточился на свежей информации
            ...
            if exploration_response:
                # Обрезаем до 500 символов - дозированная информация
                MAX_RESPONSE = 500
                ...
                # Добавляем красивое форматирование
                formatted_dive = (
                    f"{dive_emoji} <b>Углубляемся в тему</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━━\n\n"
                    f"{exploration_response}\n\n"
                    "<i>✨ Просто и понятно • Без воды</i>"
                )
                ...
                logger.info(f"✅ Погружение в тему #{clarify_count} для {user_id}: {dive_emoji}")
            else:
                await query.answer("❌ Не удалось получить информацию", show_alert=True)
                logger.warning(f"⚠️ Не удалось получить информацию для погружения {user_id}")
                
        except Exception as e:
            logger.error(f"❌ Ошибка при получении информации для погружения: {e}", exc_info=True)
