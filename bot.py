...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("clarify_"):
        ...
        try:
            # Генерируем разные аспекты в зависимости от кол-ва кликов
            aspects = [
                ("💰 Финансирование", "Про {}: Расскажи про финансирование, инвесторов и сколько они привлекли денег."),
                ("🎯 Продукты", "Про {}: Какие основные продукты и услуги предлагает этот проект?"),
                ("👥 Команда", "Про {}: Кто основатели и ключевые люди? Какой у них опыт?"),
                ("📊 Рынок", "Про {}: Какова капитализация, объем торгов и рыночная позиция?"),
                ("🤝 Партнерства", "Про {}: Какие партнерства и интеграции с другими проектами?"),
                ("🎪 Конкуренция", "Про {}: Кто главные конкуренты? Какие есть преимущества и недостатки?"),
                ("🚀 Развитие", "Про {}: Какие планы развития на ближайший год? Какие обновления?")
            ]
            
            # Выбираем аспект на основе кол-ва кликов
            aspect_index = clarify_count % len(aspects)
            aspect_emoji, aspect_template = aspects[aspect_index]
            clarify_count += 1
            
            # Получаем ответ от ИИ с конкретным аспектом БЕЗ повторения исходной информации
            from ai_dialogue import get_ai_response_sync
            
            # Вопрос формируется только про новый аспект, с явной инструкцией не повторять
            detailed_question = (
                f"ДОПОЛНИТЕЛЬНАЯ ИНФОРМАЦИЯ про {last_question}:\n\n"
                f"{aspect_template.format(last_question)}\n\n"
                f"Отвечай ТОЛЬКО на этот новый вопрос, БЕЗ повторения базовой информации. "
                f"Дай НОВЫЕ, дополнительные детали."
            )
            
            dialogue_context = None  # Без контекста, чтобы не повторять
            ...
            if detailed_response:
                # Обрезаем до 600 символов - не слишком длинно
                MAX_RESPONSE = 600
                ...
                # Добавляем структурированное форматирование
                formatted_detailed = (
                    f"{aspect_emoji} <b>Дополнительная информация</b>\n"
                    "━━━━━━━━━━━━━━━━━━━━━\n\n"
                    f"{detailed_response}\n\n"
                    "<i>🎯 Только суть без воды • Просто и понятно</i>"
                )
                
                # Добавляем кнопки для продолжения получения информации
                keyboard = [[
                ...
                # Отправляем новое сообщение с доп информацией с кнопками
                await context.bot.send_message(
                    user_id,
                    formatted_detailed,
                    parse_mode=ParseMode.HTML,
            ...
            logger.error(f"❌ Ошибка при получении доп информации: {e}", exc_info=True)
...
def main():
    ...
    try:
        ...
        # Use get_event_loop_policy().new_event_loop() for proper cleanup
        import sys
        if sys.platform == 'win32':
            asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
