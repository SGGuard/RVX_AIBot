...
class DigestFormatter:
    ...
    def format_market_sentiment(self, data: Dict) -> str:
        ...
        if avg_change < -1:  # Падающий рынок
            text += "📉 <b>Risk-OFF режим (краткосрочно)</b>\n"
            text += "→ Альты теряют быстрее BTC\n"
            text += "→ Осторожно с лонгами, фокус на поддержках\n"
        elif avg_change > 1:  # Растущий рынок
            text += "📈 <b>Risk-ON режим (краткосрочно)</b>\n"
            text += "→ Альты растут синхронно с BTC\n"
            text += "→ Хороший момент для входа в позицию\n"
        else:  # Консолидация
            text += "⏸️ <b>Консолидация (краткосрочно)</b>\n"
    ...
    def format_executive_summary(self, data: Dict) -> str:
        ...
        if avg_change < -2:
            ...
            text += "• Смотрите в Support уровни\n"
            text += "• Готовьтесь к дальнейшему падению\n"
        elif avg_change < -0.5:
            text += "⚠️ <b>Риск повышен</b>\n"
            text += "• Только сильные альты\n"
            text += "• Стоп-лосы обязательны\n"
            text += "• Без увеличения позиций\n"
        elif avg_change > 2:
            text += "✅ <b>Растущий рынок</b>\n"
            text += "• Хороший момент для входа\n"
            text += "• Откупаются альты из списка\n"
