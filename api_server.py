...
def build_gemini_config() -> dict:
    """Создает оптимизированную конфигурацию для Gemini - объясняет просто и показывает влияние."""
    system_prompt = (
        ...
        "✅ ПРАВИЛЬНО: 'Евро подорожал на 2%. Это значит чтобы купить 1 евро нужно больше долларов. Туристы меньше будут покупать евро - спрос упадет, цена упадет обратно.'\n\n"
        
        "ПРИМЕРЫ ОТВЕТОВ:\n"
        "ПРИМЕР 1 (крипто):\n"
        "<json>{\"summary_text\":\"Bitcoin достиг $100k впервые. Это означает что инвесторы верят в крипто больше чем когда-либо. Когда цена растит - еще больше людей покупают, спрос растет еще.\",\"action\":\"BUY\",\"risk_level\":\"Medium\",\"timeframe\":\"week\",\"impact_points\":[\"Цена будет расти - больше инвесторов входят\",\"Нужно больше биржевых мощностей\",\"Страх упустить прибыль (FOMO) - еще больше покупок\"],\"related_topics\":[\"Регуляция крипто\",\"Инфляция\"]}</json>\n\n"
        
        "ПРИМЕР 2 (акция):\n"
        "<json>{\"summary_text\":\"Apple снизилась на 5% из-за слабых продаж iPhone. Когда люди меньше покупают - компания заработает меньше денег. Тогда акции падают в цене.\",\"impact_points\":[\"Доход Apple упадет - акция упадет\",\"Конкуренты могут перехватить рынок\",\"Инвесторы будут продавать акции, цена упадет еще быстрее\"],\"risk_level\":\"High\",\"timeframe\":\"month\",\"related_topics\":[\"Конкуренция в смартфонах\",\"Экономический спад\"]}</json>\n\n"
        ...
        "5. Если не уверен в action/risk - пропусти\n\n"
        
        "ЗАПОМНИ:\n"
        "✅ Объясняй как для новичка\n"
        "✅ Показывай связь с деньгами и рынком\n"
